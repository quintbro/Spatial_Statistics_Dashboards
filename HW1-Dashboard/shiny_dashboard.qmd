...
createlabels <- function(x){
  sprintf("<strong>Name:</strong> %s<br/><strong>COVID Rate:</strong> %g%%", 
                  x$NAME, round(100*x$rate, 2)) %>%
  ...
  if (is.null(selection)) {
    ...
    proxy %>%
      addPolygons(data = unselected_data,
                  ...
                  label = createlabels(unselected_data),
                  layerId = ~NAME)
    ...
    proxy %>%
      addPolygons(data = selected_data,
                  ...
                  label = createlabels(selected_data),
                  layerId = ~NAME) %>%
